        ...
        stage('Download and run Images') {
            ...
            steps{
                sshagent(credentials : ['the-key']) {
                     sh '''ssh gabrielkovacs@swa-kovacs-vm2.cs.univie.ac.at "/snap/bin/docker container stop sr || true &&  /snap/bin/docker container rm sr || true &&  /snap/bin/docker pull pufarin/sr:sr-v1.0 && /snap/bin/docker run -d --name sr --restart=always  -p 8085:8085 pufarin/sr:sr-v1.0" '''
                     sh '''ssh gabrielkovacs@swa-kovacs-vm3.cs.univie.ac.at "/snap/bin/docker container stop oprop || true && /snap/bin/docker container rm oprop || true && /snap/bin/docker pull pufarin/oprop:oprop-v1.0 && /snap/bin/docker run -d --name oprop --restart=always  -p 8083:8083 pufarin/oprop:oprop-v1.0" '''
                     sh '''ssh gabrielkovacs@swa-kovacs-vm4.cs.univie.ac.at "/snap/bin/docker container stop dr || true && /snap/bin/docker container rm dr || true && /snap/bin/docker pull pufarin/dr:dr-v1.0 && /snap/bin/docker run -d --name dr --restart=always  -p 8086:8086 pufarin/dr:dr-v1.0" '''
